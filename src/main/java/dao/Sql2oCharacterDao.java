...
public class Sql2oCharacterDao implements CharacterDAO {
    ...
    public void add(Character newCharacter) {
        String sql = "INSERT INTO characters (name, race, classId, castingLevel, spellCount, preparedSpells) VALUES (:name, :race, :classId, :castingLevel, :spellCount, :preparedSpells)";
...
//        for(String key : propertiesToUpdate.keySet()){
//            String sql = "UPDATE characters SET " + key + " = :" + key + " WHERE id = :id";
//            try (Connection con = sql2o.open()) {
//                con.createQuery(sql)
//                        .addParameter(key, propertiesToUpdate.get(key))
//                        .addParameter("id", characterUpdatedId)
//                        .executeUpdate();
//            } catch (Sql2oException ex) {
//                System.out.println(ex);
